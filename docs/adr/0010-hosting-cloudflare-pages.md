...
- **Vercel / Netlify.** Same category: static SPA on a CDN, near-zero config,
  ...
  one were already in the workflow it would be the right call by "don't add a
  tool for no reason." No strong differentiator over Pages for this project.
- **Railway (or similar always-on container host).** Designed to run a server
  ...
  that does nothing a CDN doesn't do better. Rejected for V1. Note this verdict
  is workload-specific and **reverses once a real backend exists**: a container
...
- **AWS S3 + CloudFront.** Static-on-CDN, never needs migrating, and AWS would
  ...
  backend, small, edge-shaped KV/object lookups (short links, stored maps), AWS
  is more glue (Lambda + API Gateway + DynamoDB + CloudFront origin routing)
  than the equivalent on Cloudflare, with S3 egress fees where R2 has none. It
  only wins if the backend grows large and relational, which is not the
  trajectory the ADRs describe. Rejected as over-provisioning for the actual
  system.
...
- The host is **right-sized to a static SPA**: a CDN serving files, with the
  ...
  nothing to clean up if the project is paused or abandoned). This matters for a
  solo hobby project.
