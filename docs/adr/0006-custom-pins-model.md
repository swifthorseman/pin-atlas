...
annotation, not identity.** Two custom pins at the same coordinate with different
...
- **Disallow custom pins (curated-only).** Would cap usefulness to the curated
  dataset and make V1 effectively a Switzerland demo. Rejected; at least one existing product had to add custom-pin naming,
  confirming the need.
...
- Label handling is load-bearing, not edge-case: real labels are proper nouns
  with spaces, accents, apostrophes, ampersands. Enforce a length cap, URL-encode
